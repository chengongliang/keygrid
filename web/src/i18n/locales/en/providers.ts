...
const providers = {
  ...
  breakerCheck: 'Circuit breaker',
  breakerCheckHint: 'Temporarily reject requests after repeated channel failures (default: 5 failures within 30s → 30s cooldown, then an automatic probe). Keep it off when this is your only channel and the upstream is flaky: with no fallback channel to fail over to, tripping means requests just fail.',
  breakerOff: 'Off',
  breakerClosed: 'Healthy',
  breakerHalfOpen: 'Probing',
  breakerOpen: 'Tripped',
  uaLabel: 'Upstream User-Agent',
  uaModeDefault: 'Default (forward client)',
  uaModeCustom: 'Custom fixed value',
  uaModeForward: 'Force forward client (incl. Codex)',
  uaPlaceholder: 'e.g. MyApp/1.0',
  uaHint: 'By default the client User-Agent is forwarded to the upstream as-is (pi-agent / codex / claude …) so the upstream sees the real client. Codex channels keep codex_cli_rs by default (the upstream depends on it).',

  // ---- Health view (table) ----
  viewCards: 'Cards',
  viewHealth: 'Health',
  healthDesc: 'Last 24 hours. Click any row to expand its recent failures; the Actions column enables/disables the channel instantly, and resets a tripped breaker.',
  healthRowHint: 'Click to view recent failures',
  healthColChannel: 'Channel',
  healthColCred: 'Credential',
  healthColBreaker: 'Breaker',
  healthColReq24h: 'Req 24h',
  healthColFail24h: 'Fail 24h',
  healthColSuccess: 'Success',
  healthColLatency: 'Avg latency',
  healthEmpty: 'No channels yet — create one with “+ Add” in the top right',
  healthTotal: '{{total}} channel(s)',
  healthReset: 'Reset breaker',
  healthResetDone: 'Breaker state of "{{name}}" has been reset',
  healthResetFailed: 'Failed to reset the breaker',
  healthResetOff: 'Circuit breaking is off for this channel — nothing to reset',
  healthResetIdle: 'Not tripped — nothing to reset',
  healthToggleFailed: 'Failed to toggle the channel',
  healthUnauthorized: 'Not authorized yet — authorize first (no credential means no relaying)',
